...
class LLMService:
    ...
    def parse_travel_request(
        ...
        try:
            ...
            messages = [
                ...
                HumanMessage(content=user_query or f"Find flights from {departure_city} to {arrival_city} given the above conditions.")
            ]
            ...
            try:
                ...
                required_fields = ['optimal_departure_date', 'optimal_return_date', 'reasoning']
                for field in required_fields:
                    if field not in parsed_data:
                        parsed_data[field] = self._get_default_value(field, departure_range, return_range)
    ...
    def format_flight_results(self, flight_data: Dict[str, Any]) -> str:
        ...
        try:
            ...
            messages = [
                SystemMessage(content=system_prompt),
                HumanMessage(content=f"Flight Results: {json.dumps(simplified_data, indent=2)}")
            ]
    ...
    def _get_default_value(self, field: str, departure_range: tuple, return_range: tuple) -> str:
    ...
    def _get_fallback_parsed_request(self, departure_range: tuple, return_range: tuple) -> Dict[str, Any]:
    ...
    def _get_fallback_formatted_results(self, flight_data: Dict[str, Any]) -> str:
        ...
        for i, offer in enumerate(flight_data['data'][:3]):
            ...
            results.append(f"**Price:** {price.get('total', 'N/A')} {price.get('currency', 'EUR')}")
